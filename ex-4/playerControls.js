...
let controlsWrapper = document.getElementById("video-controls-wrapper");

// the buttons for the controls
let playButton = document.getElementById("playButton");
let videoPlayOverlay = document.getElementById("videoPlayOverlay");

// RETRIEVING DOM REFERENCES FOR SEEK REWIND/FORWARD 5 SECS BUTTONS (ASSESS THIS FEATURE)
...
function playPause(){
  ...
  if (videoElement.paused || videoElement.ended) {
    ...
    playButton.style.backgroundImage = "url('./icons/play.svg')";
  // SETS OVERLAY DISPLAY TO BLOCK ON USER PAUSE. 
  // PROVIDES IMMEDIATE VISUAL FEEDBACK TO USER AND SIGNIFIES THAT A CLICK AFFORDS RESUMING BY CALLING play()
...
// BELOW ADDS SUPPORT FOR CLICKING DIRECTLY ON videoPlayOverlay SINCE IT IS LIKELY THAT USERS WILL PERFORM THIS BEHAVIOUR
videoPlayOverlay.addEventListener('click', playPause);
...
function muteUnmute(){
  ...
  if(videoElement.muted){
    videoElement.muted = false; // BOOLEAN EXPRESSION TO SET AUDIO TO MUTE
...
/* SEEK REWIND/FORWARD 5 SECS (ASSESS THIS FEATURE) /*

/* SEEK REWIND/FORWARD FEATURE PROVIDES GREATER CONTROL OVER THE MEDIA AND EFFICIENCY IN NAVIGATING TO 
   A SPECIFIC PART OF A VIDEO WHICH IS A COMMON ACTION MADE BY USERS. SEEK BY USING LEFT AND RIGHT ARROW 
   KEYS (SEE KEYBOARD SHORTCUTS) OR SELECTING forwardButton OR rewindButton IN video-controls-wrapper.
   
  WHY 5 SECONDS?

  PRECISION: A 5-SECOND INTERVAL STRIKES A BALANCE BETWEEN BEING PRECISE ENOUGH FOR USERS TO SEEK, 
              WHILE STILL ALLOWING FOR QUICK NAVIGATION THROUGH LONGER MEDIA FILES.
  USER BEHAVIOR: RESEARCH HAS SHOWN THAT USERS TEND TO SEEK IN SHORT INCREMENTS, 
                  WITH 5 SECONDS BEING A COMMONLY USED DURATION POPULARISED BY 
...
rewindButton.addEventListener('click', seekRewind);

// KEYBOARD SHORTCUTS
/* IMPLEMENTING COMMON SYSTEM BEHAVIOUR PATTERN (KEYBOARD SHORTCUTS) DERIVED FROM MENTAL MODELS BASED ON USER EXPERIENCES WITH OTHER PRODUCTS (MEDIA PLAYERS) 
E.G: YOUTUBE */
document.addEventListener('keydown', function(event) {
  if (event.code === 'Space') {
     playPause(); // CALL playPause ON SPACE KEYDOWN
  }
  else if (event.code === 'ArrowLeft') {
    seekRewind(); // CALL seekRewind ON LEFT ARROW KEYDOWN
  }
  else if (event.code === 'ArrowRight') {
    seekForward(); // CALL seekForward ON RIGHT ARROW KEYDOWN
  }
});
